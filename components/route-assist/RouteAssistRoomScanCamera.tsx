...
export type RouteAssistRoomScanCaptureV1 = { version: 1; captureKind: "ORDINARY_ROOM_SCAN"; capturedAt: string; sourceLabel: string; destinationLabels: string[]; camera: { facingMode: "environment"; width: number | null; height: number | null }; sweepFrames: RouteAssistBrowserFrameV1[]; reviewFrame: Omit<RouteAssistBrowserFrameV1, "capturedAt" | "sequence"> | null };
...
export default function RouteAssistRoomScanCamera({ sourceLabel, destinationLabels, onScanComplete, onBack }: Props) {
  ...
  async function sampleSweepFrameUnlocked() {
    ...
    if (!video || sequence >= MAX_SWEEP_FRAMES) return;
    const frame = await frameFromVideo(video, sequence);
    if (!frame) return;
    if (finishingRef.current && sweepFramesRef.current.length !== sequence) { URL.revokeObjectURL(frame.objectUrl); return; }
    if (sweepFramesRef.current.length >= MAX_SWEEP_FRAMES || sweepFramesRef.current.length !== sequence) { URL.revokeObjectURL(frame.objectUrl); return; }
  ...
  async function sampleSweepFrame() {
    ...
    const run = sampleSweepFrameUnlocked().finally(() => { if (samplingPromiseRef.current === run) samplingPromiseRef.current = null; });
    samplingPromiseRef.current = run;
    return run;
  }

  async function openCamera() {
    setCameraError(null); stopSweepTimer(); finishingRef.current = false; setFinishing(false); revokeObjectUrls(); sweepFramesRef.current = []; setFrameCount(0);
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal: "environment" } }, audio: false });
      streamRef.current?.getTracks().forEach((track) => track.stop()); streamRef.current = stream;
      if (videoRef.current) { videoRef.current.srcObject = stream; await videoRef.current.play(); }
      setState("SCANNING"); await sampleSweepFrame();
  ...
  async function finishScan() {
    ...
    const capture: RouteAssistRoomScanCaptureV1 = { version: 1, captureKind: "ORDINARY_ROOM_SCAN", capturedAt: new Date().toISOString(), sourceLabel, destinationLabels: [...destinationLabels], camera: { facingMode: "environment", width: video?.videoWidth || null, height: video?.videoHeight || null }, sweepFrames: frames, reviewFrame };
    streamRef.current?.getTracks().forEach((track) => track.stop()); streamRef.current = null; if (video) video.srcObject = null;
    setState("COMPLETE"); setFinishing(false); onScanComplete(capture);
  }
  ...
  return <div className="mx-auto w-full max-w-md" data-testid="route-assist-room-scan-camera"><div className="overflow-hidden rounded-2xl border border-cardline bg-white shadow-sm">
    <div className="relative aspect-[3/4] bg-slate-950"><video ref={videoRef} muted playsInline className={`h-full w-full object-cover ${state === "SCANNING" ? "block" : "hidden"}`} aria-label="Room scan camera preview" />
    ...
    <div className="p-5"><div className="rounded-xl bg-slate-50 p-4 text-sm leading-6 text-slate"><div><strong className="text-navy">Start from:</strong> {sourceLabel}</div><div className="mt-1"><strong className="text-navy">Include:</strong> {destinationLabels.join(", ")}</div></div>{cameraError && <p className="mt-3 text-sm text-red-600">{cameraError}</p>}
      ...
      {state === "SCANNING" && <button type="button" disabled={finishing} onClick={finishScan} className="mt-4 w-full rounded-xl bg-electric px-5 py-3.5 text-sm font-semibold text-white shadow-sm disabled:opacity-60" data-testid="route-assist-finish-room-scan">{finishing ? "Finishing scan…" : "Finish scan"}</button>}
