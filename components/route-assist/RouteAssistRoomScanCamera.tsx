...
export type RouteAssistRoomScanCaptureV1 = {
  ...
  /**
   * Browser-local review frame only. It is media provenance for provider/review
   * handoff, not geometry evidence and not durable storage. The caller must
   * upload/persist it before using it across devices or sessions.
   */
  reviewFrame: { imageId: string; objectUrl: string; mimeType: "image/jpeg"; width: number; height: number } | null;
};
...
export default function RouteAssistRoomScanCamera({ sourceLabel, destinationLabels, onScanComplete, onBack }: Props) {
  ...
  async function captureReviewFrame(video: HTMLVideoElement): Promise<RouteAssistRoomScanCaptureV1["reviewFrame"]> {
    ...
    return { imageId: `browser-frame-${Date.now()}`, objectUrl, mimeType: "image/jpeg", width: canvas.width, height: canvas.height };
  }

  async function finishScan() {
    const video = videoRef.current;
    const reviewFrame = video ? await captureReviewFrame(video) : null;
    const capture: RouteAssistRoomScanCaptureV1 = {
      ...
      destinationLabels: [...destinationLabels], camera: { facingMode: "environment", width: video?.videoWidth || null, height: video?.videoHeight || null }, reviewFrame,
    };
  ...
  return (
    <div className="mx-auto w-full max-w-md" data-testid="route-assist-room-scan-camera"><div className="overflow-hidden rounded-2xl border border-cardline bg-white shadow-sm">
      <div className="relative aspect-[3/4] bg-slate-950">
        ...
        {state === "SCANNING" && <><div className="pointer-events-none absolute inset-x-4 top-4 rounded-xl bg-black/55 px-4 py-3 text-sm leading-5 text-white backdrop-blur-sm">Slowly move across the wall and keep the selected locations, baseboard, door/window trim, and other route boundaries visible when possible.</div><div className="pointer-events-none absolute left-1/2 top-1/2 h-36 w-56 -translate-x-1/2 -translate-y-1/2 rounded-2xl border-2 border-white/70" /><div className="pointer-events-none absolute inset-x-4 bottom-4 rounded-xl bg-black/55 px-4 py-3 text-center text-xs text-white/90 backdrop-blur-sm">Route Assist is capturing room scan media for geometry analysis. It will not infer hidden wiring.</div></>}
      </div>
      <div className="p-5"><div className="rounded-xl bg-slate-50 p-4 text-sm leading-6 text-slate"><div><strong className="text-navy">Start from:</strong> {sourceLabel}</div><div className="mt-1"><strong className="text-navy">Include:</strong> {destinationLabels.join(", ")}</div></div>
        ...
        {state === "COMPLETE" && <div className="mt-4 rounded-xl bg-emerald-50 p-4 text-sm font-medium text-emerald-900">Room scan captured. A review frame is ready for provider handoff; geometry still requires analysis and review.</div>}
