...
export default function RouteAssistSweepRouteReview({ frames, overlay }: Props) {
  ...
  if (!frame || !path) return <div className="rounded-xl border border-cardline bg-white p-4 text-sm text-slate">No reviewable route overlay is available. Route Assist should request another capture or contractor review.</div>;
  const polyline = path.points.map((point) => `${point.x * 400},${point.y * 300}`).join(" ");
  ...
  return <section className="overflow-hidden rounded-2xl border border-cardline bg-white shadow-sm" data-testid="route-assist-sweep-route-review">
    ...
    <div className="relative aspect-[4/3] w-full bg-slate-100">
      ...
      <svg viewBox="0 0 400 300" className="absolute inset-0 h-full w-full" role="img" aria-label="Proposed route over captured room view">
        {path.points.length > 1 && <polyline points={polyline} fill="none" stroke="#2452D9" strokeWidth="6" strokeLinecap="round" strokeLinejoin="round" />}
        {path.points.map((point, pointIndex) => <circle key={`${frame.imageId}-${pointIndex}`} cx={point.x * 400} cy={point.y * 300} r={pointIndex === 0 || pointIndex === path.points.length - 1 ? 7 : 4} fill={pointIndex === 0 ? "#0F1E3C" : "#2452D9"} />)}
      </svg>
    ...
    <div className="border-t border-cardline bg-slate-50 px-4 py-3 text-xs leading-5 text-slate">Only frames containing validated route evidence are shown. No line is drawn between different camera coordinate planes, and viewing this proposal does not accept or price the route.</div>
  </section>;
}
