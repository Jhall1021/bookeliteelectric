...
export default function QuotePricingForm({
  ...
  async function calculateRecessedLightingPackage() {
    const feet = Number(routeFeet);
    if (!Number.isFinite(feet) || feet < 1 || feet > 300) {
    ...
    setCalculating(true); setError(null); setNotice(null);
    try {
      const res = await fetch(`/api/admin/quotes/${quoteId}/recessed-lighting-scope`, {
        method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ installedCablePathFeet: feet }),
      });
      const data = await res.json().catch(() => ({})) as { error?: string; suggestedPriceCents?: number; laborHours?: number; materialCostCents?: number; lightCount?: number; cableFeet?: number; supportCount?: number };
      if (!res.ok || data.suggestedPriceCents === undefined || data.laborHours === undefined || data.materialCostCents === undefined || data.lightCount === undefined || data.cableFeet === undefined || data.supportCount === undefined) {
        throw new Error(data.error ?? "Could not calculate this reviewed recessed-lighting package.");
      }
      setCalculation({ suggestedPriceCents: data.suggestedPriceCents, laborHours: data.laborHours, materialCostCents: data.materialCostCents });
      setPrice((data.suggestedPriceCents / 100).toFixed(2));
      setNotice(`Calculated ${data.lightCount} lights from the confirmed layout, ${data.cableFeet} total cable feet, and ${data.supportCount} policy-derived supports. Confirm or edit the customer price before sending.`);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Could not calculate this reviewed recessed-lighting package.");
    } finally { setCalculating(false); }
  }

  async function calculateNewExteriorLightPackage() {
    const feet = Number(routeFeet);
    if (!Number.isFinite(feet) || feet < 1 || feet > 75) {
      setError("Enter the electrician-confirmed accessible cable path between 1 and 75 feet.");
      return;
    }
    setCalculating(true); setError(null); setNotice(null);
    try {
      const res = await fetch(`/api/admin/quotes/${quoteId}/new-exterior-light-scope`, {
        method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ accessibleRouteFeet: feet }),
      });
      const data = await res.json().catch(() => ({})) as { error?: string; suggestedPriceCents?: number; laborHours?: number; materialCostCents?: number; cableFeet?: number; supportCount?: number };
      if (!res.ok || data.suggestedPriceCents === undefined || data.laborHours === undefined || data.materialCostCents === undefined || data.cableFeet === undefined || data.supportCount === undefined) {
        throw new Error(data.error ?? "Could not calculate this reviewed exterior-light package.");
      }
      setCalculation({ suggestedPriceCents: data.suggestedPriceCents, laborHours: data.laborHours, materialCostCents: data.materialCostCents });
      setPrice((data.suggestedPriceCents / 100).toFixed(2));
  ...
  async function calculateAppliance240vPackage() {
    const feet = Number(routeFeet);
    if (!Number.isFinite(feet) || feet < 1 || feet > 50) { setError("Enter the electrician-confirmed accessible cable path between 1 and 50 feet."); return; }
    setCalculating(true); setError(null); setNotice(null);
    try {
      const res = await fetch(`/api/admin/quotes/${quoteId}/appliance-240v-scope`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ accessibleRouteFeet: feet }) });
      const data = await res.json().catch(() => ({})) as { error?: string; suggestedPriceCents?: number; laborHours?: number; materialCostCents?: number; supportCount?: number; configuration?: string };
      if (!res.ok || data.suggestedPriceCents === undefined || data.laborHours === undefined || data.materialCostCents === undefined || data.supportCount === undefined) throw new Error(data.error ?? "Could not calculate this reviewed appliance-circuit package.");
      setCalculation({ suggestedPriceCents: data.suggestedPriceCents, laborHours: data.laborHours, materialCostCents: data.materialCostCents });
      setPrice((data.suggestedPriceCents / 100).toFixed(2));
      setNotice(`Calculated the ${data.configuration ?? "selected appliance"} package from confirmed scope and ${data.supportCount} policy-derived supports. Confirm or edit the customer price before sending.`);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not calculate this reviewed appliance-circuit package."); }
    finally { setCalculating(false); }
  }

  async function calculateEvChargerPackage() {
    const feet = Number(routeFeet);
    if (!Number.isFinite(feet) || feet < 1 || feet > 50) { setError("Enter the electrician-confirmed accessible cable path between 1 and 50 feet."); return; }
    setCalculating(true); setError(null); setNotice(null);
    try {
      const res = await fetch(`/api/admin/quotes/${quoteId}/ev-charger-scope`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ accessibleRouteFeet: feet }) });
      const data = await res.json().catch(() => ({})) as { error?: string; suggestedPriceCents?: number; laborHours?: number; materialCostCents?: number; supportCount?: number; configuration?: string };
      if (!res.ok || data.suggestedPriceCents === undefined || data.laborHours === undefined || data.materialCostCents === undefined || data.supportCount === undefined) throw new Error(data.error ?? "Could not calculate this reviewed EV charger package.");
      setCalculation({ suggestedPriceCents: data.suggestedPriceCents, laborHours: data.laborHours, materialCostCents: data.materialCostCents });
      setPrice((data.suggestedPriceCents / 100).toFixed(2));
      setNotice(`Calculated the ${data.configuration ?? "hardwired EV charger"} package from confirmed scope and ${data.supportCount} policy-derived supports. Confirm or edit the customer price before sending.`);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not calculate this reviewed EV charger package."); }
    finally { setCalculating(false); }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-5 rounded-card border border-cardline bg-warmwhite p-4 sm:p-5">
      ...
      {recessedLightingStandardReview && (
        <div className="mb-5 rounded-card border border-blue-100 bg-blue-50 p-4">
          <p className="text-xs font-bold uppercase tracking-[0.12em] text-electric">Reviewed accessible recessed-lighting package</p>
          <h3 className="mt-1 font-display text-base font-bold text-navy">Confirm the existing source and actual attic cable path</h3>
          <p className="mt-1 text-sm text-slate">The customer requested {recessedLightingCount} light{recessedLightingCount === 1 ? "" : "s"}. Continue only after confirming an ordinary accessible route and a suitable existing switched-lighting feed. Enter the complete installed cable path from that source through every new light. Do not substitute the old ten-feet-per-light allowance.</p>
          <div className="mt-3 flex flex-wrap items-end gap-3">
            <label className="text-sm font-semibold text-navy">Confirmed cable path feet
              <input type="number" min="1" max="300" step="0.1" value={routeFeet} onChange={(event) => setRouteFeet(event.target.value)} className="mt-1 block w-36 rounded-card border border-cardline bg-white px-3 py-2 text-sm" />
            </label>
            <button type="button" onClick={() => { void calculateRecessedLightingPackage(); }} disabled={calculating} className="rounded-pill bg-electric px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{calculating ? "Calculating…" : "Confirm layout and calculate"}</button>
          </div>
          {calculation && calculation.laborHours > 0 && <p className="mt-3 text-xs text-slate">Suggested ${(calculation.suggestedPriceCents / 100).toFixed(2)} · {calculation.laborHours.toFixed(2)} crew-hours · ${(calculation.materialCostCents / 100).toFixed(2)} direct material</p>}
        </div>
      )}
      {newExteriorLightStandardReview && (
        <div className="mb-5 rounded-card border border-blue-100 bg-blue-50 p-4">
          <p className="text-xs font-bold uppercase tracking-[0.12em] text-electric">Reviewed one-location exterior-light package</p>
          <h3 className="mt-1 font-display text-base font-bold text-navy">Confirm the source, wall, and actual accessible cable path</h3>
          <p className="mt-1 text-sm text-slate">Continue only after confirming one customer-supplied hardwired fixture, ordinary first-story siding/sheathing, a suitable existing switched source, and no remediation. The homeowner&apos;s rough distance is context only—enter the complete installed path through the accessible attic, basement, or crawlspace.</p>
          <p className="mt-2 text-xs text-slate">Includes the exterior fixture box, ordinary wall penetration and sealing, cable, fixture installation, testing, and cleanup. Excludes masonry/stucco specialty work, new controls, lifts, finish repair, remediation, and additional locations.</p>
          <div className="mt-3 flex flex-wrap items-end gap-3">
            <label className="text-sm font-semibold text-navy">Confirmed cable path feet
              <input type="number" min="1" max="75" step="0.1" value={routeFeet} onChange={(event) => setRouteFeet(event.target.value)} className="mt-1 block w-36 rounded-card border border-cardline bg-white px-3 py-2 text-sm" />
            </label>
      ...
      {appliance240vStandardReview && (
        <div className="mb-5 rounded-card border border-blue-100 bg-blue-50 p-4">
          <p className="text-xs font-bold uppercase tracking-[0.12em] text-electric">Reviewed four-wire appliance circuit</p>
          <h3 className="mt-1 font-display text-base font-bold text-navy">Confirm the appliance, panel, surface box, and actual cable path</h3>
          <p className="mt-1 text-sm text-slate">Continue only after the appliance instructions and plug confirm the selected 30A dryer or 50A range package, the panel can accept the circuit without remediation, and a surface-mounted box behind the appliance is suitable. Enter the actual attic, unfinished-basement, crawlspace, or drop-ceiling path—not the homeowner&apos;s rough range.</p>
          <p className="mt-2 text-xs text-slate">Three-prong legacy outlets, hardwired equipment, wall ovens, cooktops, flush-wall boxes, finished or inaccessible routes, panel work, and routes over 50 feet remain manual review.</p>
          <div className="mt-3 flex flex-wrap items-end gap-3">
            <label className="text-sm font-semibold text-navy">Confirmed cable path feet
              <input type="number" min="1" max="50" step="0.1" value={routeFeet} onChange={(event) => setRouteFeet(event.target.value)} className="mt-1 block w-36 rounded-card border border-cardline bg-white px-3 py-2 text-sm" />
            </label>
            <button type="button" onClick={() => { void calculateAppliance240vPackage(); }} disabled={calculating} className="rounded-pill bg-electric px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{calculating ? "Calculating…" : "Confirm scope and calculate"}</button>
          </div>
          {calculation && calculation.laborHours > 0 && <p className="mt-3 text-xs text-slate">Suggested ${(calculation.suggestedPriceCents / 100).toFixed(2)} · {calculation.laborHours.toFixed(2)} crew-hours · ${(calculation.materialCostCents / 100).toFixed(2)} direct material</p>}
        </div>
      )}
      {evChargerStandardReview && (
        <div className="mb-5 rounded-card border border-blue-100 bg-blue-50 p-4">
          <p className="text-xs font-bold uppercase tracking-[0.12em] text-electric">Reviewed hardwired EV charger</p>
          <h3 className="mt-1 font-display text-base font-bold text-navy">Confirm the charger rating, panel, mounting location, and actual cable path</h3>
          <p className="mt-1 text-sm text-slate">Continue only after the charger instructions confirm a 40A-output hardwired unit on a 50A circuit, the panel can accept it without remediation or load management, the ordinary attached-garage wall is suitable, and the actual accessible route is confirmed. Enter that measured route—not the homeowner&apos;s rough range.</p>
          <p className="mt-2 text-xs text-slate">Plug-in chargers, outdoor or detached locations, pedestals, specialty walls, app/network setup, finished or inaccessible routes, panel work, and routes over 50 feet remain manual review.</p>
          <div className="mt-3 flex flex-wrap items-end gap-3">
            <label className="text-sm font-semibold text-navy">Confirmed cable path feet
              <input type="number" min="1" max="50" step="0.1" value={routeFeet} onChange={(event) => setRouteFeet(event.target.value)} className="mt-1 block w-36 rounded-card border border-cardline bg-white px-3 py-2 text-sm" />
            </label>
