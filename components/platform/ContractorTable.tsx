        ...
        <tbody className="divide-y divide-cardline">
          {rows.map((r) => (
            <tr key={r.id}>
              ...
              <td className="px-4 py-2 tabular-nums">{r.live}</td>
              <td className="px-4 py-2">{r.canLaunch ? "passes" : `${r.blockers} blocker${r.blockers === 1 ? "" : "s"}`}</td>
              <td className="px-4 py-2">{r.site ? `${r.site.active ? "live" : "inactive"} · /${r.site.hostedSlug}${r.site.embedOriginsConfigured ? ` · ${r.site.embedOriginsConfigured} embed origin${r.site.embedOriginsConfigured === 1 ? "" : "s"}` : ""}` : "none"}</td>
