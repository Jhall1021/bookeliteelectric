...
ok(ELECTRICAL_LABOR_SCOPE_FACTS.length === 48 && Object.keys(ELECTRICAL_LABOR_SCOPE_COLLECTION_GROUPS).length === 29, "the 48 labor facts collapse into 29 reusable collection groups");
ok([...requiredFacts].every((key) => plannedFacts.has(key)), "the plan covers every fact needed by all 34 affected services");
...
ok(branchPlan.find((task) => task.collectionGroupKey === "ACCESSIBLE_ROUTE_MEASUREMENT")?.collectionPath === "CUSTOMER_TREE", "accessible attic, basement and crawlspace paths ask for an approximate homeowner distance first");
