...
ok(panel.includes("Panel and breaker service totals") && panel.includes("PANEL_AND_BREAKER_SERVICE_SLUGS"), "panel and breaker work is grouped by complete service with atomic detail available underneath");
ok(route.includes("saveLaborOperationDecisions"), "edited atomic units remain the labor authority");
ok(route.includes("contractorLaborOperationDecision.createMany") && route.includes("skipDuplicates: true"), "prepared labor units are inserted in one bounded statement instead of sequential transaction upserts");
ok(route.includes("timeout: 20_000"), "the unified labor transaction has an explicit production-safe timeout");
