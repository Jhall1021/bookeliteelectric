...
ok(ROUTING_V2_LABOR_AUTHORITY.filter((entry) => entry.runtimeUsesAtomicDecision).every((entry) => ["ELEC_ROUTE_SURFACE_MOUNTED", "ELEC_ROUTE_BACK_TO_BACK", "SURFACE_ROUTE_FT", "SURFACE_ROUTE_INSIDE_CORNER", "SURFACE_ROUTE_OUTSIDE_CORNER", "SURFACE_ROUTE_FLAT_CORNER", "OUTLET_EXTENSION_CORE", "SURFACE_DEVICE_BOX_OUTLET", "SWITCH_ENDPOINT_CORE", "SURFACE_DEVICE_BOX_SWITCH", "FIXTURE_BOX_ENDPOINT", "SURFACE_FIXTURE_BOX"].includes(entry.componentKey)), "only implemented surface endpoints and back-to-back routing claim atomic runtime adoption");

const summary = summarizeRoutingV2LaborBridge();
ok(summary.componentCount === 18 && summary.exactButUnwiredCount === 0 && summary.compositeUnwiredCount === 6, "summary distinguishes connected route paths from six remaining composite mappings");
ok(summary.runtimeConnectedCount === 12, "twelve component types now report atomic runtime adoption");
