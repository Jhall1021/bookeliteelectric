...
const same = (row: { boundaries: number[]; choice: string | null }, answer: { boundaries?: readonly number[]; choice?: string }) =>
...
async function main() {
  ...
  try {
    ...
    const rows = await db.contractorPolicyValue.findMany({
      ...
      select: { key: true, boundaries: true, choice: true, resolvedAt: true },
    });
    ...
    for (const item of ANSWERS) {
      ...
      const proposed = "choice" in item.answer ? item.answer.choice : item.answer.boundaries.join(", ");
      if (row.resolvedAt) {
