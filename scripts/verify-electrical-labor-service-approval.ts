...
ok(route.includes('service.isPrimaryEligible ? "PRIMARY"') && route.includes('? { fieldLaborHours: projection.suggestedHours }') && route.includes(': { wwtLaborHours: projection.suggestedHours }'), "add-on-only services store their atomic duration as add-on labor");
...
ok(page.includes("service.isPrimaryEligible ? service.fieldLaborHours : service.wwtLaborHours"), "setup compares each suggestion with the labor field its price actually consumes");
