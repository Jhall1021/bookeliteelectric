...
ok(priceable.filter((row) => row.runtimeConnection === "CONNECTED").length === 56, "40 bounded services, seven policy/review-bound connected-device packages, the new-outlet route pilot, three surface-route services, two standard accessible low-voltage packages, the reviewed 15A dedicated-circuit package and entry path, and the reviewed accessible new-ceiling-light package have atomic runtime pricing paths");
