...
ok(priceable.filter((row) => row.missingScopeFacts.length > 0).length === 41, "41 priceable services name unresolved physical scope facts rather than hiding them in flat hours");
...
ok(priceable.filter((row) => row.runtimeConnection === "CONNECTED").length === 40, "35 bounded services, four policy-bounded connected devices, and the new-outlet route pilot have atomic runtime pricing paths");
