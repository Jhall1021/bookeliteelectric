...
ok(priceable.filter((row) => row.runtimeConnection === "CONNECTED").length === 54, "40 bounded services, seven policy/review-bound connected-device packages, the new-outlet route pilot, three surface-route services, two standard accessible low-voltage packages and the reviewed 15A dedicated-circuit package have atomic runtime pricing paths");
