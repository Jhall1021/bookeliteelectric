...
ok(priceable.filter((row) => row.missingScopeFacts.length > 0).length === 40, "40 priceable services name unresolved physical scope facts rather than hiding them in flat hours");
...
ok(priceable.filter((row) => row.runtimeConnection === "CONNECTED").length === 41, "36 bounded services, four policy-bounded connected devices, and the new-outlet route pilot have atomic runtime pricing paths");
