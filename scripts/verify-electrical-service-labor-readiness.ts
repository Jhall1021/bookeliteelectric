...
ok(priceable.filter((row) => row.runtimeConnection === "CONNECTED").length === 57, "40 bounded services, seven policy/review-bound connected-device packages, the new-outlet route pilot, three surface-route services, two standard accessible low-voltage packages, the reviewed 15A dedicated-circuit package and entry path, and the reviewed accessible new-ceiling-light/fan packages have atomic runtime pricing paths");
...
ok(rows.find((row) => row.serviceSlug === "new-ceiling-light")?.runtimeConnectionReason.includes("existing switched-light source"), "new ceiling light reports only its bounded reviewed accessible package as connected");
ok(rows.find((row) => row.serviceSlug === "new-ceiling-fan")?.runtimeConnectionReason.includes("existing switched-light source"), "new ceiling fan reports only its bounded reviewed accessible package as connected");
