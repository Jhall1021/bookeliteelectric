...
ok(serviceUnknown.kind === "INCOMPLETE" && serviceUnknown.missingQuantities.includes("ELEC_SERVICE_ENTRANCE_CONDUCTOR") && serviceUnknown.missingQuantities.includes("ELEC_PULL_OVERHEAD_SERVICE_CONDUCTOR") && serviceUnknown.missingQuantities.includes("ELEC_DRIVE_GROUNDING_ELECTRODE") && serviceUnknown.missingQuantities.includes("ELEC_TERMINATE_GROUNDING_ELECTRODE_CLAMP") && serviceUnknown.missingQuantities.includes("ELEC_ROUTE_GROUNDING_ELECTRODE_CONDUCTOR") && serviceUnknown.missingQuantities.includes("condition:difficultGroundingConditions"), "service upgrade refuses until feeder, overhead-conductor and decomposed grounding quantities are known");
const serviceReady = evaluateLaborRecipe(serviceUpgrade, { serviceEntranceFeet: 20, overheadServiceConductorFeet: 30, groundingElectrodeCount: 2, groundingClampTerminationCount: 3, groundingElectrodeConductorFeet: 25, difficultGroundingConditions: false, singlePoleCircuitCount: 17, doublePoleCircuitCount: 3 }, calibrated);
ok(serviceReady.kind === "READY" && serviceReady.quantities.ELEC_SERVICE_ENTRANCE_CONDUCTOR === 20 && serviceReady.quantities.ELEC_INSTALL_OVERHEAD_SERVICE_MAST === 1 && serviceReady.quantities.ELEC_INSTALL_SERVICE_WEATHERHEAD === 1 && serviceReady.quantities.ELEC_INSTALL_METER_HUB === 1 && serviceReady.quantities.ELEC_INSTALL_SERVICE_MAST_SUPPORT_SET === 1 && serviceReady.quantities.ELEC_PULL_OVERHEAD_SERVICE_CONDUCTOR === 30 && serviceReady.quantities.ELEC_DRIVE_GROUNDING_ELECTRODE === 2 && serviceReady.quantities.ELEC_TERMINATE_GROUNDING_ELECTRODE_CLAMP === 3 && serviceReady.quantities.ELEC_ROUTE_GROUNDING_ELECTRODE_CONDUCTOR === 25 && serviceReady.quantities.ELEC_DIFFICULT_GROUNDING_ELECTRODE_INSTALL === undefined, "service-upgrade labor carries the explicit mast assembly, per-foot overhead conductors, meter-to-panel feeder and decomposed ordinary grounding scope");
