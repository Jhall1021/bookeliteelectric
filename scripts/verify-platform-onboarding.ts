...
async function main() {
  ...
  try {
    ...
    const a3 = await attachOwnerFor(db, staff, probeId, other.email);
    ok(`   an account that already owns another business may be attached too — the one-business rule is lifted for platform-attached owners (7 Sep 2026)`, a3.ok && !a3.already && (await raw.contractorMembership.count({ where: { contractorId: probeId, role: "OWNER", active: true } })) === 2);
    ok(`   while the self-serve create path keeps its own one-business guard`, /ALREADY_OWNS/.test(strip("lib/contractorCreation.ts")) && !/ALREADY_OWNS/.test(strip("lib/platformOnboarding.ts")));
    ...
    const idxR = await onboardingIndexFor(db, staff);
