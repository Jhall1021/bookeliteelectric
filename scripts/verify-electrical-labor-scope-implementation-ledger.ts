...
ok(rows.length === 24, "all 24 currently required authority-specific grouped collection tasks have an implementation row");
...
ok(panelCapacity.state === "PARTIAL_RUNTIME_CONNECTION" && panelCapacity.runtimeConnectedServiceSlugs.join() === "240v-garage-outlet,240v-garage-outlet-14-30,240v-garage-outlet-14-50,240v-garage-outlet-6-50,bidet-smart-toilet-outlet,dedicated-120v-circuit-outlet,electric-fireplace-circuit,freezer-fridge-dedicated-circuit,sump-pump-dedicated-circuit", "panel capacity connects the reviewed 15A entries, plug-in fireplace, sump-specific 20A package and four exact open-garage 240V configurations while other circuit scope remains review-bound");
...
ok(routeAccess.length === 2 && routeAccessConnected.join() === "240v-garage-outlet,240v-garage-outlet-14-30,240v-garage-outlet-14-50,240v-garage-outlet-6-50,bidet-smart-toilet-outlet,dedicated-120v-circuit-outlet,electric-fireplace-circuit,exterior-gfci-other-routing,freezer-fridge-dedicated-circuit,garage-door-opener-outlet,garage-door-opener-outlet-ev,new-120v-outlet,new-ceiling-fan,new-ceiling-light,new-coax-line,new-ethernet-line,new-exterior-flood-camera,new-exterior-lighting-locations,new-wall-sconce,recessed-lighting,sump-pump-dedicated-circuit", "route access separates customer-tree access from guided review while recording only the connected bounded branches");
