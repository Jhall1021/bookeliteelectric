...
ok(new Set(rows.flatMap((row) => row.serviceSlugs)).size === 34, "implementation ledger covers all 34 services with unresolved scope facts");
...
ok(panelCapacity.state === "PARTIAL_RUNTIME_CONNECTION" && panelCapacity.runtimeConnectedServiceSlugs.join() === "240v-garage-outlet,240v-garage-outlet-14-30,240v-garage-outlet-14-50,240v-garage-outlet-6-50,bidet-smart-toilet-outlet,dedicated-120v-circuit-outlet,electric-fireplace-circuit,freezer-fridge-dedicated-circuit,level-2-ev-charger,new-240v-appliance-circuit,sump-pump-dedicated-circuit" && panelCapacity.note.includes("nonblocking confirmation photos"), "panel-capacity handling gives bounded circuit families a price while retaining review for failed assumptions and higher-risk packages");
const applianceCircuit = rows.find((row) => row.collectionGroupKey === "APPLIANCE_CIRCUIT_REVIEW")!;
ok(applianceCircuit.state === "PARTIAL_RUNTIME_CONNECTION" && applianceCircuit.runtimeConnectedServiceSlugs.join() === "new-240v-appliance-circuit" && applianceCircuit.note.includes("legacy three-wire"), "appliance configuration prices exact modern four-wire dryer and range outcomes while keeping exceptions in review");
...
ok(routeAccess.length === 2 && routeAccessConnected.join() === "240v-garage-outlet,240v-garage-outlet-14-30,240v-garage-outlet-14-50,240v-garage-outlet-6-50,bidet-smart-toilet-outlet,dedicated-120v-circuit-outlet,electric-fireplace-circuit,freezer-fridge-dedicated-circuit,garage-door-opener-outlet,garage-door-opener-outlet-ev,level-2-ev-charger,new-120v-outlet,new-240v-appliance-circuit,new-coax-line,new-ethernet-line,new-exterior-flood-camera,new-exterior-lighting-locations,recessed-lighting,sump-pump-dedicated-circuit", "route access separates customer-tree access from guided review while recording only services that still require runtime collection");
