...
ok(lightingCount.state === "PARTIAL_RUNTIME_CONNECTION" && lightingCount.runtimeConnectedServiceSlugs.join() === "recessed-lighting" && lightingCount.note.includes("whole-number"), "customer-selected recessed-light count connects without opening broad exterior-lighting scope");
...
ok(lightingSource.state === "PARTIAL_RUNTIME_CONNECTION" && lightingSource.runtimeConnectedServiceSlugs.join() === "new-ceiling-fan,new-ceiling-light,new-wall-sconce,recessed-lighting", "existing lighting-source suitability connects only through contractor review of the bounded lighting packages");
...
ok(routeAccess.length === 2 && routeAccessConnected.join() === "240v-garage-outlet,240v-garage-outlet-14-30,240v-garage-outlet-14-50,240v-garage-outlet-6-50,bidet-smart-toilet-outlet,dedicated-120v-circuit-outlet,exterior-gfci-other-routing,freezer-fridge-dedicated-circuit,garage-door-opener-outlet,garage-door-opener-outlet-ev,new-120v-outlet,new-ceiling-fan,new-ceiling-light,new-coax-line,new-ethernet-line,new-exterior-flood-camera,new-wall-sconce,recessed-lighting,sump-pump-dedicated-circuit", "route access separates customer-tree access from guided review while recording only the connected bounded branches");
