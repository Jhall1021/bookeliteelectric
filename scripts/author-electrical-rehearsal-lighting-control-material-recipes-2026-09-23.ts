/**
 * Install the reviewed physical material recipes for the shared lighting
 * control components on the designated rehearsal branch. Canonical recipes
 * carry quantities only; contractor prices remain contractor-owned.
 *
 * Existing nonmatching recipe rows are never overwritten. A mismatch fails
 * closed so a later run cannot silently redefine physical scope.
 */
...
async function main() {
  ...
  try {
    const componentKeys = LIGHTING_CONTROL_MATERIAL_RECIPES.map((recipe) => recipe.componentKey);
    const materialKeys = [...new Set(LIGHTING_CONTROL_MATERIAL_RECIPES.flatMap((recipe) => recipe.lines.map((line) => line.role)))];
    ...
    for (const role of ["NM_CABLE_SUPPORT", "BOX_OLD_WORK", "SWITCH_STANDARD", "WALL_PLATE"]) {
      if (materialsByKey.get(role)?.unit !== "each") throw new Error(`${role} must be priced per used item`);
    }

    console.log(`\nELECTRICAL LIGHTING-CONTROL MATERIAL RECIPES — ${apply ? "AUTHOR" : "REPORT"}`);
    console.log(`  target: ${identity.endpoint}\n`);
    const pending = [] as typeof LIGHTING_CONTROL_MATERIAL_RECIPES[number][];
    for (const recipe of LIGHTING_CONTROL_MATERIAL_RECIPES) {
      ...
      if (current.length > 0 && signature(current) !== signature(recipe.lines)) {
        throw new Error(`${recipe.componentKey} already has a different physical recipe; refusing to overwrite it`);
      }
      if (current.length === 0) pending.push(recipe);
      console.log(`  ${recipe.componentKey}: ${current.length ? "already current" : apply ? "author" : "would author"}`);
      for (const line of recipe.lines) console.log(`    ${line.quantity} ${materialsByKey.get(line.role)!.unit} ${line.role}`);
    }
    if (!apply) {
      console.log(`\n  would author ${pending.length} recipe(s); no change\n`);
      return;
    }
    ...
    console.log(`\n  authored ${pending.length} canonical component recipe(s)\n`);
  } finally {
    await db.$disconnect();
  }
}
