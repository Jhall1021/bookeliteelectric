    ...
    <main className="min-h-screen bg-warmwhite px-4 py-6" data-testid="route-assist-camera-demo">
      <div className="mx-auto w-full max-w-md">
        <header className="mb-5">
          ...
          <p className="mt-2 text-sm leading-6 text-slate">This fixture previews phone capture and homeowner review without changing production pricing or guided flows.</p>
        </header>

        <div className="mb-4 grid grid-cols-3 gap-2" data-testid="route-assist-camera-kind">
          {(Object.keys(LABELS) as DemoKind[]).map((option) => (
            <button key={option} type="button" onClick={() => chooseKind(option)} className={`rounded-xl border px-2 py-2.5 text-xs font-semibold ${kind === option ? "border-electric bg-blue-50 text-electric" : "border-cardline bg-white text-slate"}`}>{LABELS[option]}</button>
          ))}
        </div>
        ...
        {capture && (
          <section className="mt-4 overflow-hidden rounded-2xl border border-cardline bg-white shadow-sm" data-testid="route-assist-wiremold-review">
            <div className="border-b border-cardline p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-slate">Route Assist prefers the trim-hugging path rather than a shorter run across the open wall.</p>
            </div>

            <div className="p-4">
              <RouteAssistSurfaceRacewayPreview destinationKind={kind} />

              <div className="mt-4 rounded-xl bg-blue-50 p-3 text-sm text-navy">
              ...
              {reviewState === "PENDING" && (
                <div className="mt-4 grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setReviewState("ADJUST")} className="rounded-xl border border-cardline bg-white px-3 py-3 text-sm font-semibold text-navy" data-testid="route-assist-adjust-route">Adjust route</button>
                  <button type="button" onClick={() => setReviewState("ACCEPTED")} className="rounded-xl bg-electric px-3 py-3 text-sm font-semibold text-white" data-testid="route-assist-looks-good">Looks good</button>
                </div>
              )}

              {reviewState === "ACCEPTED" && <div className="mt-4 rounded-xl bg-emerald-50 p-3 text-sm font-medium text-emerald-900" data-testid="route-assist-route-reviewed">Route reviewed. This fixture records the homeowner checkpoint only; it does not bind pricing or material quantities.</div>}
              ...
              {reviewState === "ADJUST" && (
                <div className="mt-4 rounded-xl bg-amber-50 p-3 text-sm leading-5 text-amber-950" data-testid="route-assist-route-adjustment">
                  ...
                  <button type="button" onClick={() => setReviewState("PENDING")} className="mt-2 block text-xs font-semibold underline underline-offset-4">Return to review</button>
                </div>
              )}
            </div>
          </section>
