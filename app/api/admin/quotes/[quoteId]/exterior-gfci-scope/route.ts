...
const FIXED_MATERIAL_ROLES = ["GFCI_WEATHER_RESISTANT", "COVER_IN_USE_BUBBLE", "BOX_FS_CAST", "CONSUMABLES_SMALL"] as const;
const MATERIAL_ROLES = [...FIXED_MATERIAL_ROLES, "WIRE_12_2", "NM_CABLE_SUPPORT"] as const;
...
export async function PATCH(req: Request, { params }: { params: { quoteId: string } }) {
  return withAdminRoute(async (db, ctx) => {
    ...
    if (typeof routeFeet !== "number" || !Number.isFinite(routeFeet) || routeFeet < 1 || routeFeet > 20) {
      return NextResponse.json({ error: "Confirmed accessible route must be between 1 and 20 feet." }, { status: 400 });
    }
    ...
    if (quote.service.slug !== SLUG) return NextResponse.json({ error: "This is not the reviewed routed exterior-GFCI package." }, { status: 409 });
    const answers = quote.answersSnapshot as Record<string, string>;
    if (!isReviewedAccessibleExteriorGfci(answers)) {
      return NextResponse.json({ error: "This request falls outside the reviewed accessible exterior-GFCI package." }, { status: 409 });
    }
    ...
    if (FIXED_MATERIAL_ROLES.some((role) => (serviceQuantity.get(role) ?? 0) <= 0)) {
      return NextResponse.json({ error: "Approve the GFCI, weatherproof enclosure and consumables quantities before calculating this package." }, { status: 409 });
    }
    const materialCostCents = assembleMaterialCostCents([
      ...
      { unitCostCents: costs.get("WIRE_12_2")!, quantity: cableFeet },
    ...
    const basisFingerprint = createHash("sha256").update(JSON.stringify({
      ...
      cableFeet,
      supportCount,
      contractorConfirmedOrdinarySourceAndExteriorWall: true,
