...
import { isReviewedAccessibleNewWallSconce } from "@/lib/electrical/newCeilingLightReviewPackage";
...
const SLUG = "new-wall-sconce";
const FIXED_MATERIAL_ROLES = ["BOX_OLD_WORK", "CONSUMABLES_SMALL"] as const;
...
export async function PATCH(req: Request, { params }: { params: { quoteId: string } }) {
  return withAdminRoute(async (db, ctx) => {
    ...
    if (quote.service.slug !== SLUG) return NextResponse.json({ error: "This is not the reviewed new-wall-sconce package." }, { status: 409 });
    const answers = quote.answersSnapshot as Record<string, string>;
    if (!isReviewedAccessibleNewWallSconce(answers)) {
      return NextResponse.json({ error: "This request falls outside the reviewed accessible new-wall-sconce package." }, { status: 409 });
    }
    ...
    if (FIXED_MATERIAL_ROLES.some((role) => (serviceQuantity.get(role) ?? 0) <= 0)) {
      return NextResponse.json({ error: "Approve the wall-box and consumables quantities before calculating this package." }, { status: 409 });
    }
