...
const FIXED_MATERIAL_ROLES = ["BOX_EXTERIOR_FIXTURE", "CONSUMABLES_SMALL"] as const;
...
export async function PATCH(req: Request, { params }: { params: { quoteId: string } }) {
  return withAdminRoute(async (db, ctx) => {
    ...
    if (typeof routeFeet !== "number" || !Number.isFinite(routeFeet) || routeFeet < 1 || routeFeet > 75) {
      return NextResponse.json({ error: "Confirmed accessible cable path must be between 1 and 75 feet." }, { status: 400 });
    }
    ...
    if (quote.service.slug !== SLUG) return NextResponse.json({ error: "This is not the reviewed one-location exterior-light package." }, { status: 409 });
    const answers = quote.answersSnapshot as Record<string, string>;
    if (!isReviewedAccessibleNewExteriorLight(answers)) {
      return NextResponse.json({ error: "This request falls outside the reviewed one-location exterior-light package." }, { status: 409 });
    }
    ...
    const [policies, materialRows, serviceMaterials, storedDecisions, settings] = await Promise.all([
      ...
      db.contractorMaterial.findMany({
        where: { contractorId: ctx.contractorId, active: true, canonicalMaterial: { key: { in: [...MATERIAL_ROLES] } } },
    ...
    if (MATERIAL_ROLES.some((role) => !costs.has(role))) {
      return NextResponse.json({ error: "Enter costs for the exterior box, cable, cable supports, and consumables before calculating this package." }, { status: 409 });
    }
    ...
    if (FIXED_MATERIAL_ROLES.some((role) => fixedQuantities.get(role) !== 1)) {
      return NextResponse.json({ error: "The one-location exterior box and consumables recipe is incomplete." }, { status: 409 });
    }
    ...
    const basisFingerprint = createHash("sha256").update(JSON.stringify({
      ...
      exteriorLightCount: 1,
      existingLightingSourceConfirmed: true,
      ordinaryExteriorWallConfirmed: true,
    ...
    const touched = await db.quote.updateMany({
      ...
      data: {
        reviewFactsSnapshot: {
          ...
          exteriorLightCount: { value: 1, source: "CUSTOMER_TREE" },
          existingLightingSourceConfirmed: { value: true, source: "GUIDED_PHOTO_REVIEW" },
          ordinaryExteriorWallConfirmed: { value: true, source: "GUIDED_PHOTO_REVIEW" },
        },
    ...
    return NextResponse.json({ ok: true, suggestedPriceCents: suggestion.totalCents, laborHours: labor.suggestedHours, materialCostCents, routeFeet, cableFeet, supportCount, basisFingerprint, sent: false });
  });
}
