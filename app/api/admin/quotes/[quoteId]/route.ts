...
export async function PATCH(req: Request, { params }: { params: { quoteId: string } }) {
  ...
  return withAdminRoute(async (db, ctx) => {
    ...
    const { quotedPriceCents, depositRequired } = body;
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Price must be a positive whole amount in cents." },
        { status: 400 }
      );
    }
    if (depositRequired !== undefined && typeof depositRequired !== "boolean") {
      return NextResponse.json(
        { error: "Deposit required must be true or false." },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      emailError = "The price was saved, but the customer email could not be sent.";
      console.error(
        `[quotes] priced ${quote.id} but the email to ${quote.customer.email} failed: ${detail}`
      );
    }
    ...
    return NextResponse.json({
      ...
      // phone, rather than assuming it went out. Internal provider details stay
      // in server logs instead of crossing the API boundary.
      emailError,
    });
  });
}
