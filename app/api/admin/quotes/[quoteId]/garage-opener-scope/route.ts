...
const SLUG = "garage-door-opener-outlet";
const FIXED_MATERIAL_ROLES = ["RECEPTACLE_STANDARD", "BOX_OLD_WORK", "WALL_PLATE", "CONSUMABLES_SMALL"] as const;
...
export async function PATCH(req: Request, { params }: { params: { quoteId: string } }) {
  return withAdminRoute(async (db, ctx) => {
    ...
    if (quote.service.slug !== SLUG) return NextResponse.json({ error: "This is not the reviewed garage-opener package." }, { status: 409 });
    const answers = quote.answersSnapshot as Record<string, string>;
    if (!isReviewedGarageOpenerRequest(answers)) {
      return NextResponse.json({ error: "This request falls outside the garage-opener review flow." }, { status: 409 });
    }
    ...
    if (FIXED_MATERIAL_ROLES.some((role) => (serviceQuantity.get(role) ?? 0) <= 0)) {
      return NextResponse.json({ error: "Approve the receptacle, box, plate, and consumables quantities before calculating this package." }, { status: 409 });
    }
    ...
    const basisFingerprint = createHash("sha256").update(JSON.stringify({
      ...
      permitAdminCents: quote.service.permitAdminCents, otherDirectCostCents: quote.service.otherDirectCostCents, settings,
    })).digest("hex");
