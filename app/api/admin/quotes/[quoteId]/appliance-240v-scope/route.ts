...
const SHARED_FIXED_ROLES = ["BOX_SURFACE_4S", "COVER_RAISED_4S", "CONSUMABLES_MEDIUM"] as const;
...
export async function PATCH(req: Request, { params }: { params: { quoteId: string } }) {
  return withAdminRoute(async (db, ctx) => {
    ...
    const config = reviewedAppliance240vConfiguration(quote.service.slug, quote.answersSnapshot as Record<string, string>);
    if (!config) return NextResponse.json({ error: "This request falls outside the reviewed four-wire dryer/range package." }, { status: 409 });

    const materialRoles = [config.receptacleRole, config.breakerRole, ...SHARED_FIXED_ROLES, config.wireRole, "NM_CABLE_SUPPORT"] as const;
    ...
    if (materialRoles.some((role) => !costs.has(role))) return NextResponse.json({ error: "Enter costs for every selected appliance-circuit material before calculating its price." }, { status: 409 });
    const quantities = new Map(serviceMaterials.flatMap((line) => line.quantity !== null && line.canonicalMaterial
      ...
      : []));
    if (SHARED_FIXED_ROLES.some((role) => quantities.get(role) !== 1)) return NextResponse.json({ error: "The appliance surface box, cover, and consumables recipe is incomplete." }, { status: 409 });

    const materialCostCents = assembleMaterialCostCents([
      { unitCostCents: costs.get(config.receptacleRole)!, quantity: 1 },
    ...
    const labor = projectElectricalServiceLabor(quote.service.slug, decisions, {
      ...
      applianceCircuitConfigurationConfirmed: true,
    });
    ...
    const basisFingerprint = createHash("sha256").update(JSON.stringify({
      ...
      panelCapacityConfirmed: true, applianceCircuitConfigurationConfirmed: true,
    ...
    const touched = await db.quote.updateMany({ where: { id: quote.id, status: { in: ["SUBMITTED", "IN_REVIEW"] } }, data: {
      reviewFactsSnapshot: {
        ...
        applianceCircuitConfigurationConfirmed: { value: true, source: "GUIDED_PHOTO_REVIEW" },
      },
    ...
    return NextResponse.json({
      ...
      configuration: `${config.appliance} ${config.amperage}A four-wire`, basisFingerprint, sent: false,
    });
  });
}
