...
const SLUG = "new-ceiling-light";
const FIXED_MATERIAL_ROLES = ["BOX_CEILING_STANDARD", "CONSUMABLES_SMALL"] as const;
...
export async function PATCH(req: Request, { params }: { params: { quoteId: string } }) {
  return withAdminRoute(async (db, ctx) => {
    ...
    if (quote.service.slug !== SLUG) return NextResponse.json({ error: "This is not the reviewed new-ceiling-light package." }, { status: 409 });
    const answers = quote.answersSnapshot as Record<string, string>;
    if (!isReviewedAccessibleNewCeilingLight(answers)) {
      return NextResponse.json({ error: "This request falls outside the reviewed accessible new-ceiling-light package." }, { status: 409 });
    }
    ...
    if (FIXED_MATERIAL_ROLES.some((role) => (serviceQuantity.get(role) ?? 0) <= 0)) {
      return NextResponse.json({ error: "Approve the ceiling-box and consumables quantities before calculating this package." }, { status: 409 });
    }
