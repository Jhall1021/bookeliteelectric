...
export const ELECTRICAL_OWNER_APPROVED_STARTING_MINUTES_2026_09_25: Record<string, number> = {
  ...
  ELEC_INSTALL_SMART_SWITCH_HARDWARE: 17.4,
  ELEC_INSTALL_SMART_RECEPTACLE_HARDWARE: 15,
...
const PLANNING_SEEDS: Record<string, PlanningSeed> = {
  ...
  ELEC_INSTALL_SMART_SWITCH_HARDWARE: { hoursPerUnit: 0.29, sourceKeys: ["SERVICE:customer-supplied-smart-switch", "SERVICE:replace-led-dimmer"], note: "A customer-supplied smart switch uses the same bounded hardware-replacement time as a compatible dimmer; connected-device commissioning remains separate." },
  ELEC_INSTALL_SMART_RECEPTACLE_HARDWARE: { hoursPerUnit: 0.25, sourceKeys: ["SERVICE:smart-outlet-upgrade", "SERVICE:replace-standard-outlet"], note: "A compatible smart receptacle uses the same bounded hardware-replacement time as a standard receptacle; connected-device commissioning remains separate." },
