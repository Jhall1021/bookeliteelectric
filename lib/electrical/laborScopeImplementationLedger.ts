...
export const ELECTRICAL_SCOPE_GROUP_IMPLEMENTATION: Record<string, GroupImplementation> = {
  ...
  ACCESSIBLE_ROUTE_MEASUREMENT: {
    ...
    note: "The ordinary new-outlet route prices from the homeowner's approximate point-to-point accessible distance plus the contractor's standard end allowance. Reviewed dedicated-circuit, equipment and lighting packages may still require contractor-confirmed measurements because they carry additional technical facts; cable-support counts come from contractor policy. Ethernet and coax may use the contractor-approved maximum footage for their standard accessible package. Other affected services still await binding. Route Assist remains reserved for inaccessible finished-space or surface routes.",
  },
  ...
  EV_CHARGER_CONFIGURATION_REVIEW: {
    state: "PARTIAL_RUNTIME_CONNECTION",
    evidencePaths: ["prisma/seed-level-2-ev-charger.ts", "lib/electrical/evChargerReviewPackage.ts", "app/api/admin/quotes/[quoteId]/ev-charger-scope/route.ts"],
    note: "Only the reviewed customer-supplied hardwired 40A-output charger on a 50A circuit connects after contractor confirmation of the instructions, panel, ordinary attached-garage mounting and actual accessible route. Plug-in, outdoor, detached, load-managed, specialty-wall, commissioning and remediation scopes remain manual review.",
  },
  ...
  SPECIALTY_EQUIPMENT_TAKEOFF: {
    ...
    note: "The spa path records contractor-confirmed bonding applicability, measured bonding-conductor footage and exact accessible connection count. Uncertain bonding remains review-only.",
  },
