...
function providerPrompt(request: RouteAssistHttpVisibleSceneRequestV1, media: readonly RouteAssistAiGatewayMediaV1[]): string {
  return [
    ...
    "Box format: x/y are the normalized top-left corner; width/height are normalized extents. Every box must satisfy 0<=x<1, 0<=y<1, 0<width<=1-x, 0<height<=1-y -- never emit a box extending beyond the image. If an object is partly cropped by the image edge, box only the visible portion inside the image. Do not use pixel coordinates, and do not emit 0..100 or 0..1000 scale coordinates. Example of a valid box: {\"x\":0.25,\"y\":0.10,\"width\":0.30,\"height\":0.70}. Confidence is evidentiary only.",
