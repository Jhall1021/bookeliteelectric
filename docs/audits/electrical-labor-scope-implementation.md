...
| CONNECTED_DEVICE_SCOPE | CUSTOMER_TREE | PARTIAL_RUNTIME_CONNECTION | customer-supplied-smart-switch, floodlight-camera-existing, smart-outlet-upgrade, smart-thermostat-install, video-doorbell-existing-wiring | 2 | lib/electrical/connectedDeviceLaborFacts.ts; lib/electrical/laborServiceApproval.ts; app/api/portal/labor-service-review/route.ts |
...
- **CONNECTED_DEVICE_SCOPE / CUSTOMER_TREE:** An explicitly resolved contractor commissioning policy binds five compatible connected-device packages to atomic duration review. Smart-thermostat customer pricing still requires blocking wiring review before the clean no-remediation package can apply.
