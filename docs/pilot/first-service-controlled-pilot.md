...
**Recorded defect, not fixed here (native capacity):** native capacity (`bookedPerWindow`) counts bookings whose `ArrivalWindow.date` equals the day's UTC midnight, but checkout stores the schedule page's timestamp for that day (`nextWorkingDays` keeps the time of day), so storefront bookings are never counted and a window at a one-job capacity stays bookable. `verify-derived-scheduling-browser` prints this as a NOTE and proves the "Fully booked" label against a booking stored under the date key capacity reads.
