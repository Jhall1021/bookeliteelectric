...
**Recorded findings, not fixed here:** (1) only the schedule page's server-rendered FIRST day applies the duration — another day's tab refetches `GET /api/availability/[date]?duration=…`, and that route does not read `duration`, so a later day can offer a window checkout then refuses with `WINDOW_TOO_LATE`; (2) a window withheld for job length is labelled "Fully booked", which is not the reason.
